...
class Agent(object):
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            ...
            for path in re_n:
                rev_q = [0]
                for reward in reversed(path):
                    rev_q.append(reward + self.gamma*rev_q[-1])
                q_n.extend(rev_q[:0:-1])
            q_n = np.array(q_n)
        else:
            q_n = []
            for path in re_n:
                path_rew = 0
                for reward in reversed(path):
                    path_rew = reward + self.gamma * path_rew
                q_n.extend(len(path)*[path_rew])
            q_n = np.array(q_n)
        return q_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        p, _ = self.sess.run((self.sy_logprob_n,self.update_op), {self.sy_ob_no : ob_no,self.sy_ac_na : ac_na, self.sy_adv_n : adv_n})
        #print(p[:5])
        # print(self.sess.run(self.loss, {self.sy_ob_no : ob_no,self.sy_ac_na : ac_na, self.sy_adv_n : adv_n}))
...
def train_PG(
    ...
    for itr in range(n_iter):
        ...
        # test = tf.get_default_graph().get_tensor_by_name("loss:0")
        # print(test)
